...
contract CashRegister {
    ...
    // receiptNonce is incremented to assign a unique receiptID to each receipt struct
    uint receiptNonce;

    // Each ndividual receipts takes the form of a struct
    struct Receipt {
        address purchaser;
        uint receiptID;
        uint totalPrice;
    ...
    modifier restricted() {
        require(msg.sender == owner, "Only the owner call call this function");
        _;
    }

    // used by owner to add items to his/her store to make them available to a purchaser
    // requires itemName a string, and itemPrice as a uint
    function addItem(string itemName, uint itemPrice) public restricted {
        // any item added must have a price more than 0, otherwise it will not be found in the items mapping 
        require(itemPrice > 0, "itemPrice must be more than 0"); 
        // adding the item to the items mapping
        // The itemName is the key, and the itemPrice is the value returned
        items[keccak256(abi.encodePacked(itemName))] = itemPrice;
    }

    // used by the owner to create a new receipt, and in the process emit a NewReceipt event
    function newReceipt(address purchaser) public restricted {
        // which includes the purchasers address and a unique ID for the receipt
        ...
        emit NewReceipt(purchaser, receiptNonce);
        // create new receipt utilixing the Receipt struct
        // add the new receipt to the receipts mapping, with requires values assigned to the variables
        receipts[receiptNonce] = Receipt({purchaser: purchaser, receiptID: receiptNonce, totalPrice: 0, finished: false});
    }
