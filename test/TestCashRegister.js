...
contract('CashRegister', async(accounts) => {
    ...
    it('should create a new receipt that includes the purchasers address and a unique ID', async() => {
        ...
        const receiptIDFromLogs = transReceipt.logs[0].args._receiptID.toString()
        ...
        const receiptIDInStruct = receiptStruct[1].toString();
    ...
    it('should allow the purchaser to add an item to their receipt', async() => {
        ...
        const receiptID = transReceipt.logs[0].args._receiptID.toString();
        ...
        const totalPriceOnReceipt = receiptStruct[2].toString();

        assert.strictEqual(itemPriceInMapping.toString(), totalPriceOnReceipt, 'setPrice variable is not the same as totalPrice variable found on the receipt');
    })
    ...
    it('should allow the purchaser to add multiple items to their receipt', async() => {
        ...
        // Map over all the items in grocery list, and add all items to the items mapping, sending the transactions from the managers address
        ...
        const purchaserFromLogs = transReceipt.logs[0].args._purchaser.toString();
        const receiptIDFromLogs = transReceipt.logs[0].args._receiptID.toString();
        // Map over all the items in the grocery list and add all items to the purchasers recipt, sending the transaction from the purchasers address
    ...
    it('should return the purchasers address and total price found on a given receipt when viewReceipt is called', async() => {
        ...
        assert.strictEqual(setPrice, returnedTotalPrice.toString(), 'totalPrice returned from viewReceipt is not the same as setPrice');
    ...
    it('should allow the manager to claim all tokens in the CashRegister', async() => {
        ...
        const tokensDebitedFromContract = contractBalanceBeforeTransfer.sub(contractBalanceAfterTransfer).toString();
        // Calculate amount of tokens credited to the manager
        const tokensCreditedToManager = managerBalanceAfterTransfer.sub(managerBalanceBeforeTransfer).toString();
